...
class AccountTransactionControllerTest (
    ...
    @DisplayName("""
        ...
        4. 삭제
    """)
    fun accountTransactionFlowTest(){
        val createAccTracRequestDto = CreateAccTracRequestDto(
            1,
            "ADD",
            1000,
            "테스트",
        ...
        val createResult = mvc.perform(post("/api/v1/transactions/account")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(createAccTracRequestDto)))
        ...
        mvc.perform(get("/api/v1/transactions/account/$transactionId"))
            .andExpect(status().isOk)
        ...
        mvc.perform(delete("/api/v1/transactions/account/$transactionId"))
            .andExpect(status().isOk)
